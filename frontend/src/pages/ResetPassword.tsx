...
export default function ResetPassword() {
  ...
  const { register, handleSubmit, formState: { errors }, watch } = useForm<ResetPasswordData>({
    ...
    defaultValues: { 
      email,
      // Default to empty strings to avoid uncontrolled input issues
      newPassword: '',
      confirmPassword: ''
    }
  });
      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-dark-card p-10 rounded-2xl shadow-xl border border-slate-100 dark:border-dark-border">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-slate-600 dark:text-slate-400">
            {step === 1 ? `Enter the OTP sent to ${email}.` : 'Enter your new password.'}
          </p>
        </div>

        {verifyOtpMutation.isError && step === 1 && (
          <div className="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-3 rounded-lg text-sm text-center">
            Invalid OTP or it has expired.
          </div>
        )}

        {resetPasswordMutation.isError && step === 2 && (
          <div className="bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-3 rounded-lg text-sm text-center">
        ...
        {step === 1 && (
          <div className="mt-8 space-y-6">
            ...
            <button
              onClick={onVerifyOtp}
              disabled={verifyOtpMutation.isPending || !currentOtp || currentOtp.length < 6}
            ...
            >
              {verifyOtpMutation.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Verify OTP'}
            </button>
          </div>
        )}
              ...
              <div>
                ...
                <div className="mt-1 relative">
                  ...
                  <input
                    ...
                    className="appearance-none block w-full pl-10 pr-10 px-3 py-2 border border-slate-300 dark:border-dark-border rounded-lg shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent dark:bg-dark-bg dark:text-white sm:text-sm transition-colors"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
